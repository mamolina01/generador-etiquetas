...
export const InputImage = ({ onFileInputChange, logo }) => {
	...
	useEffect(() => {
		...
		return () => URL.revokeObjectURL(logo[0]?.preview);
	}, []);
				...
				<>
					<div className="w-fit mx-auto flex flex-col justify-center gap-2">
						<div className=" mx-auto w-fit p-1 border-2 border-zinc-200">
							<img src={logo[0]?.preview} className="w-28 h-28 object-cover" />
						</div>
						<p
							onClick={() => onFileInputChange([])}
							className="w-full text-center bg-red-500 p-1 text-white rounded-sm mx-auto"
						>
							Eliminar
						</p>
					</div>
